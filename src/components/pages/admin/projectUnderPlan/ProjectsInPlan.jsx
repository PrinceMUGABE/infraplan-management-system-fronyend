...
function ManageProjectsInPlan() {
  ...
  const handleAcceptProject = async (id) => {
    try {
      const response = await fetch(`http://127.0.0.1/planned/accept/${id}/`, {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        throw new Error('Error accepting project plan');
      }
      const data = await response.json();
      console.log(data);
    } catch (error) {
  ...
  const handleRejectProject = async (id) => {
    try {
      const response = await fetch(`http://127.0.0.1/planned/reject/${id}/`, {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        throw new Error('Error rejecting project plan');
      }
      const data = await response.json();
      console.log(data);
    } catch (error) {
